...
Our first post talked about why most enterprise AI Agent projects stall: not because the model isn't smart enough, but because of a "judgment gap" — teams aren't sure which decisions should go to AI. A recent piece in Berkeley's California Management Review makes almost the same point from a different angle. Its argument is direct: once data is abundant and models are commoditized, a company's real edge stops being its data or its models. It becomes the judgment senior people build up over years but can never quite explain — what's usually called tacit knowledge.[^1]
